...
NAME                = f'SnS_Inv_mexp_130425_Resnet50_robustL2_i500_readoutONconv51_subsample1000_v2'
#WITHIN_PARETO_ORDER = 'onevar'#'onevar'
GLOBAL_RSEED        = 50000
ITER                = 500
...
ROBUST_VARIANT      = 'imagenet_l2_3_0.pt'#'Liu2023Comprehensive_ConvNeXt-B'    imagenet_l2_3_0.pt
LOW_LY              = 52
HIGH_LY             = 56
NOISE_STRENGTH     = 0.01
Task2Bound = {
...
#these are the neurons in common betw vanilla and robust
# -- READOUT --
...
#Get the restricted space for the restricted space version of SnS
SPACE_SIZE = 1000
NUM_SPACES = 10
...
def get_SnS_restricted_space_args():
    ...
    args = [
        (rs, f'{HIGH_LY}={"["+n.replace("[", "(").replace("]", ")").replace(",", "")+"]" if "," in n else n}',
        f'G={GEN_VARIANT}, L={HIGH_LY}, N={n}, S={nrs}',
        Task2Sign[task], Task2Bound[task], Task2NatStat[task])
        for rs in RND_SEED
        for n,nrs in zip(NEURONS,RSEEDS)
    ...
    rec_scr_high = '"' + "#".join([str(n)     for _,  n, _,   _,           _ in args])+ '"'
    ref_p        = '"' + "#".join([str(ref)   for _,  _, ref, _,           _ in args])+ '"'
    signatures   = '"' + "#".join([str(sign)  for _,  _, _,   sign,        _ in args])+ '"'
    bounds       = '"' + "#".join([str(bound) for _,  _, _,   _,    bound, _ in args]) + '"'
    nat_thresh   = '"' + "#".join([str(nt)    for _,  _, _,   _,    _,     nt in args]) + '"'
    
    return rand_seeds, rec_scr_high, ref_p, signatures, bounds, nat_thresh  
 
def get_SnS_restricted_space_args_txt():
    ...
    args = [
        ...
        for n,nrs in zip(NEURONS,RSEEDS)
        for task in TASK
        for space in TXT_SPACE
        #NOTE: here you can also parse SPACE_STRINGS instead of TXT_SPACE,
        #but it will take way longer to be parsed (hours instead of seconds)
    ]
...
def get_SnS_restricted_space_args_spaceZIPrs():
...
if __name__ == '__main__':
    ...
    print('[3] SnS multi experiment - restricted space & rand init')
    print('[4] SnS multi experiment - restricted space & rand init - txt')
    print('[5] SnS multi experiment - restricted space & rand init - spaceZIPrs')
    ...
    match choice:
        ...
        case 1:
            ...
            rand_seeds, rec_score_ly, ref_p, signatures, bounds, nat_thresh = get_bmm_args()
        ...
        case 3:
            rand_seeds, rec_scr_high, ref_p, signatures, bounds, nat_thresh = get_SnS_restricted_space_args()
            
            args[str(ArgParams.RandomSeed)]                 = rand_seeds
            args[str(ExperimentArgParams.Rec_high)]         = rec_scr_high
            args[str(ExperimentArgParams.Score_high)]       = rec_scr_high
            args[str(ExperimentArgParams.ReferenceInfo)]    = ref_p
            args[str(ExperimentArgParams.ScoringSignature)] = signatures
            args[str(ExperimentArgParams.Bounds)]           = bounds
            args[str(ExperimentArgParams.Nrec_aggregate)]   = nat_thresh
            
            args[str(ExperimentArgParams.Rec_low)]  = '"'+f'{LOW_LY}='+SPACE_STRINGS[0]+'"'
            args[str(ExperimentArgParams.Score_low)] = '"'+f'{LOW_LY}='+SPACE_STRINGS[0]+'"'
            args[str(ExperimentArgParams.Within_pareto_order)] = 'onevar'
            
            file = 'run_multi_rand_init.py'
            
        case 4:
            rand_seeds, rec_score_ly, ref_p, signatures, bounds, nat_thresh = get_SnS_restricted_space_args_txt()
        ...
        case 5:
            rand_seeds, rec_score_ly, ref_p, signatures, bounds, nat_thresh = get_SnS_restricted_space_args_spaceZIPrs()
    ...
    #args[str(ExperimentArgParams.Within_pareto_order)] = WITHIN_PARETO_ORDER
